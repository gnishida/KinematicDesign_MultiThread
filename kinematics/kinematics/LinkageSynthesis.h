...
namespace kinematics {
	...
	public:
		...
		static std::vector<glm::dmat3x3> perturbPoses(const std::vector<glm::dmat3x3>& poses, std::pair<double, double>& sigmas, std::default_random_engine& generator, double& position_error, double& orientation_error);
		...
		void particleFilterThread(int thread_id, const std::vector<glm::dmat3x3>& poses, std::vector<Solution>& particles, const cv::Mat& dist_map, const BBox& dist_map_bbox, const std::vector<glm::dvec2>& linkage_avoidance_pts, const Object25D& moving_body);
		void resample(std::vector<Solution> particles, int N, std::vector<Solution>& resampled_particles, double max_cost, std::default_random_engine& generator);
